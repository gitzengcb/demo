...
//    public static TestrecordsController testrecordsController=new TestrecordsController();
//    public static ReportController reportController=new ReportController();
    ...
    @Autowired
    TestrecordsController testrecordsController;
...
//                        .body(caselist.getAsserts(),equalTo(caselist.getAssertresult()))
    ...
    private void responseAsserts(Response response,Cases cases,variable variablelist) {
        ...
        String returnresults=response.body().print();
        if (response.body().print().length()>=10000){
            returnresults="返回数据过长，无法存储";
        }
        ...
        try {
            if (path.getInt("code") == 200) {
                ...
                if (cases.getAsserttype().equals("=")) {
                    if (sb.toString().equals(cases.getAssertresult())) {
                        ...
                        //输出参数提取
                        if (jsonOutputParameter.size()>0||!jsonOutputParameter.isEmpty()) {
            ...
            testrecordsController.assertresultinsert(returnresults, cases, variablelist,report);
            ...
            cases.setAssertionresults(false);
            testrecordsController.assertresultinsert("异常捕获:" + e, cases, variablelist,report);
            reportController.addreport(cases,report);
        }
    ...
    private boolean casestart(Performtasks performtasks, variable variablelist) {
        ...
        //创建执行报告任务
        report= reportController.reportinsert(performtasks,report);
//        report.setId(reportid);
        ...
        for (String sceneid : split) {
            ...
            for (Object caseids:caseidlist){
                caselist.add(casesService.casestart(Integer.valueOf(caseids.toString())));
            }
    ...
    private Integer casesums(String scenegroupid) {
        ...
        for (String sceneid : split) {
            //组装场景下的caseid
            if (caselists(sceneid)==null){
                logger.info(sceneid+"没有用例case或该场景停用");
            }else {
                List caseidlist = caselists(sceneid);
    ...
    private List caselists( String sceneid) {
        ...
        if (scenecase.equals(null)) {
            logger.info( "场景id为"+sceneid+"+的场景不存在或停用");
            System.out.println( "场景id为"+sceneid+"+的场景不存在或停用");
            return null;
        }else {
            String casegroup = scenecase.getCasegroup();
            if(!scenecase.getCasegroup().isEmpty()){
...
//        map.put("starttime", LocalDateTime.now().plusDays(-7));
